...
pub fn build_dashboard_text(app: &App) -> Text<'_> {
    ...
    for tickr in &app.tickrs {
        if let Some(interval) = tickr.intervals.iter().find(|i| i.end_time.is_none()) {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(&tickr.description, Style::default().fg(Theme::text()).add_modifier(Modifier::BOLD)),
            ]));
            lines.push(Line::from(vec![
                Span::raw("    "),
    ...
    let today_tickrs: Vec<_> = app.tickrs
        .iter()
        .filter(|tickr| {
            tickr.intervals.iter().any(|interval| {
                interval.start_time >= today_start
            })
        })
        .collect();
    
    let today_duration = today_tickrs.iter().fold(Duration::seconds(0), |acc, tickr| {
        let tickr_duration = tickr.intervals.iter().fold(Duration::seconds(0), |acc2, interval| {
    ...
    let today_projects: std::collections::HashSet<_> = today_tickrs
        .iter()
    ...
    if app.tickrs.is_empty() {
        ...
        for tickr in recent_tickrs {
            let total_duration = tickr.intervals.iter().fold(Duration::seconds(0), |acc, interval| {
                let end_time = interval.end_time.unwrap_or(now);
